...
function showForm(container, existing) {
  ...
  render(container, el('div', { class: 'card' }, [
    ...
    el('div', { class: 'field', style: 'margin-top:8px' }, [
      ...
      el('p', { class: 'hint', style: 'margin:2px 0 6px' }, '会社名・TEL・許可作業のレ点（チェック）・備考など、計画にない項目をここで定義します。種類「チェック(レ点)」はチェックすると Excel に ✓ が入ります。種類「○で1つ選択」は、各選択肢名のセルに {{故障休止}}{{点検休止}}{{調整休止}} のようにタグを置くと、選ばれたものだけ「○故障休止」のように選択肢名＋○が入ります（選択肢名は残るので用紙のラベルとしてそのまま使えます）。種類「ハンコ(赤丸印)」は苗字を入力すると、そのタグのセル（例 {{担当者印}}）に赤丸の印影画像が入ります。'),
      fieldsBox,
