...
- **項目の並び**: 号機別の総運転時間（`総運転時間1〜3`・数値・hr）は空気圧縮機の記録なので「圧縮機・温水」のヘッダー圧力の直後に置く。油面確認1〜3 は「油面確認」セクション。並びを変えたときは `_schema.js` の `UTILITY_MIGRATIONS` に「移動前の値」を条件にした UPDATE を足す（投入済みDBにも効き、再実行しても no-op。管理画面での並び替えを上書きしない）
