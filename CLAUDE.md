...
- **項目の並び**: 号機別の総運転時間（数値・hr）と油面確認（選択式）は空気圧縮機の記録なので、「圧縮機・温水」のヘッダー圧力の直後に**号機ごとに1組**（総運転時間1／油面確認1／総運転時間2／…）で並べる。セクションは 灯油系統 / 圧縮機・温水 / 各種タンク / 蒸気ボイラー・ドレン の4つで、**`sort_order` はセクションごとに番号帯を分ける**（灯油1〜・圧縮機10〜・各種タンク30〜・蒸気40〜）。帯が重なるとセクションが分断されるため、項目を足すときは帯の中で採番すること（画面側も同じセクション名は1枚のカードにまとめるので保険はある）。並びを変えたときは `_schema.js` の `UTILITY_MIGRATIONS` に「移動元になり得る位置」を条件にした UPDATE を足す（投入済みDBにも効き、どの旧レイアウトからでも1回で揃い、再実行しても no-op。管理画面での並び替えを毎回は上書きしない）
