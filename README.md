...
**現在の構成はすべて無料枠内で動作します。ただし下記の上限を超えると自動で課金が発生します（上限で止まるのではなく課金される点に注意）。**

| サービス | 無料枠の上限 | 超過時 |
|---|---|---|
| Pages | リクエスト無制限・ビルド500回/月 | $5/月〜 |
| Workers（Pages Functions） | **10万リクエスト/日** | $0.30/100万リクエスト |
| D1 | **5GB・読取500万行/日・書込10万行/日** | $0.001/GB など |
| R2 | **10GB** ストレージ | $0.015/GB/月 |
| Cloudflare Access | **ユーザー50名まで無料** | 51名目から$7/ユーザー/月 |

10名・小規模運用であれば通常は無料枠内に収まります。課金リスクがあるとすれば R2 への動画大量保存のみです。アプリ側で **8GB 警告 / 9GB アップロード拒否**のガードを実装済みです。

**推奨設定:** Cloudflare ダッシュボード → Notifications → Billing Alerts で使用量アラートを設定しておくと安心です。
